...
BER_CSI_Sim_s1 = zeros(N_realization, length(SNR));
BER_CSI_theo_s1 = zeros(N_realization, length(SNR));

BER_CSI_Sim_s2 = zeros(N_realization, length(SNR));
BER_CSI_theo_s2 = zeros(N_realization, length(SNR));
...
for i = 1:length(SNR)
    ...
    P_t = 10 ^ (SNR(i) / 10) * N / mean(abs(Hd(:, 1) .^ 2));
%     P_t = sqrt(10 ^ (SNR(i) / 10) * N / sum(abs(Hd(:, 1) .^ 2)));%calculate the translate power
%     P_t = 10 ^ (SNR(i) / 10) * N / mean(abs(Hd(:, 1) .^ 2)) / 100;
    
    xn_add_cp1 = xn_add_cp .* sqrt(P_t);
    % P1 = mean(abs(xn_add_cp).^2);

    %% modulate signal with c(n)
    xn_add_cp_1 = reshape(xn_add_cp1, N_cp + N_FFT, N_frame);
    xn_add_cp_2 = xn_add_cp_1 .* cn;
    xn_add_cp_3 = reshape(xn_add_cp_2, 1, (N_cp + N_FFT) * N_frame);
    
    xn_fading_d = conv(xn_add_cp1,hd);% pass through the hd
    xn_fading_d = xn_fading_d(:, 1:length(xn_add_cp1));
...
%     BER_LSE_theo_p(jj, i) = result;
    ...
    BER_CSI_theo_s(jj, i) = qfunc(sqrt(2 * P_t .* sum(abs(Hb(:, 1).^ 2)) / N));

    %% BER Performance with Method1(Second transmission)
    gamma_c1 = P_t * sum(abs(Hb(:, 1).^ 2)) / (N * (1.5 + 0.75 * N_FFT * N / P_t / sum(abs(Hb(:, 1).^ 2))));
    BER_CSI_theo_s1(jj, i) = qfunc(sqrt(2 * gamma_c1));

    %% BER Performance with Method2(Second transmission)
    gamma_c2 = P_t * sum(abs(Hb(:, 1).^ 2)) / (N * (1.5 + 0.75 * Lch * N / P_t / sum(abs(Hb(:, 1).^ 2))));
    BER_CSI_theo_s2(jj, i) = qfunc(sqrt(2 * gamma_c2));
...
%         else 
%             H_est_ls_spline = y_remove_cp ./ xn_add_pilot;
%         end
%     end

    %%%%%%Channel Estimation for primary transmission%%%%%%
    H_est_LSE = primary_H_est(y_remove_cp, pilot_loc, P_t, F_p, N_FFT, N_frame, F_L);

    %%%%%%Channel Estimation for second transmission(Method 1)%%%%%%
%     H_est_LSE1 = estimate_1(xn_add_pilot, y_remove_cp, P_t, F_L, N_FFT, Lch);

    %%%%%%Channel Estimation for second transmission(Method 2)%%%%%%
%     H_est_LSE2 = estimate_2(xn_add_pilot, y_remove_cp, P_t, F_L, N_FFT, Lch);
...
%     y_demodulated_CSI = qpskdemod(r_remove_pilot_CSI, N_symbol, N_frame);
    ...
    yn = qpskmod(y_demodulated_LS_linear, N_symbol, N_frame);
    yn = reshape(yn, N_symbol, N_frame);
%     xn_pilot_signal = ones(1, N_pilot_number);
    yn_1 = add_pilot(yn, xn_pilot_signal, pilot_loc, N_pilot_number, N_symbol, N_frame);

    %%%%%%Channel Estimation for second transmission(Method 1)%%%%%%
    H_est_LSE1 = estimate_1(yn_1, y_remove_cp, P_t, F_L, N_FFT, Lch, N_frame);

    %%%%%%Channel Estimation for second transmission(Method 2)%%%%%%
    H_est_LSE2 = estimate_2(yn_1, y_remove_cp, P_t, F_L, N_FFT, Lch, N_frame);
...
%     BER_LSE_Sim_p(jj, i) = Neb_lslinear / Ntb;
    ...
    %%%%%%估计Hd和Hb%%%%%%
    Hd_est_1 = 0.5 .* (H_est_LSE1(:, 1) + H_est_LSE1(:, 2));
%     P_hb3 = mean(abs(Hd_est_1) .^ 2);
    Hb_est_1 = 0.5 .* (H_est_LSE1(:, 1) - H_est_LSE1(:, 2));
%     P_hb4 = mean(abs(Hb_est_1) .^ 2);

    Hd_est_2 = 0.5 .* (H_est_LSE2(:, 1) + H_est_LSE2(:, 2));
%     P_hb5 = mean(abs(Hd_est_2) .^ 2);
    Hb_est_2 = 0.5 .* (H_est_LSE2(:, 1) - H_est_LSE2(:, 2));
%     P_hb6 = mean(abs(Hb_est_2) .^ 2);

    %%%%%%估计c(n)并计算BER%%%%%%

    c_estimated1 = arg_min(H_est_LSE1, Hd_est_1, Hb_est_1, N_frame);
    Neb_CSI_s1 = sum(sum(c_estimated1 ~= cn));
    BER_CSI_Sim_s1(jj, i) = Neb_CSI_s1 / N_frame;

    c_estimated2 = arg_min(H_est_LSE2, Hd_est_2, Hb_est_2, N_frame);
    Neb_CSI_s2 = sum(sum(c_estimated2 ~= cn));
    BER_CSI_Sim_s2(jj, i) = Neb_CSI_s2 / N_frame;
...
BER_CSI_Sim_s1 = sum(BER_CSI_Sim_s1, 1) ./ N_realization;
BER_CSI_theo_s1 = sum(BER_CSI_theo_s1, 1) ./ N_realization;

BER_CSI_Sim_s2 = sum(BER_CSI_Sim_s2, 1) ./ N_realization;
BER_CSI_theo_s2 = sum(BER_CSI_theo_s2, 1) ./ N_realization;

%% 画图
figure(1);
% semilogy(SNR, BER_CSI_Sim_s, 'y-*');
% hold on;
% semilogy(SNR, BER_CSI_theo_s, 'm-s');
% hold on;
semilogy(SNR, BER_CSI_Sim_s1, 'r-o');
hold on;
% semilogy(SNR, BER_CSI_theo_s1, 'k-^');
% hold on;
semilogy(SNR, BER_CSI_Sim_s2, 'b-*');
hold on;
% semilogy(SNR, BER_CSI_theo_s2, 'c-v');
% hold on;
